...
function(project_get_directory_name _out_name)
	get_filename_component(_curr_directory_name "${CMAKE_CURRENT_SOURCE_DIR}" NAME)
	set(${_out_name} ${_curr_directory_name} PARENT_SCOPE)
endfunction()

#######################################################################
# Retrieves the current target name from the name of the current directory, and stores the result in _out_target_name
# project_get_target_name(<output variable>)
function(project_get_target_name _out_target_name)
	project_get_directory_name(_curr_directory_name)
	set(${_out_target_name} ${_curr_directory_name} PARENT_SCOPE)
endfunction()
...
# #######################################################################
# Given a list of file paths (absolute or relative), adds each file to a source group (Visual Studio filter directory)
# such that the source group matches the folder path relative to the specified _root_path
#
# - Adapted from answer in http://stackoverflow.com/questions/31422680/how-to-set-visual-studio-filters-for-nested-sub-directory-using-cmake
...
function(project_add_prep _out_sources_and_headers)
  ...
  file(GLOB _sources_and_headers
    "*.h"
    "*.hpp"
    "*.tcc"
    "*.cpp"
  ...
  foreach(_subdirectory IN ITEMS ${_subdirectory_list})
    ...
    if(NOT EXISTS ${_subdirectory}/CMakeLists.txt)
      ...
      file(GLOB_RECURSE _this_glob_header
        "${_subdirectory_name}/*.h "
        "${_subdirectory_name}/*.hpp"
        "${_subdirectory_name}/*.tcc")
  ...
  # source_group(TREE ${CMAKE_CURRENT_SOURCE_DIR} FILES ${_sources_and_headers})
  # source_group(TREE ${CMAKE_CURRENT_SOURCE_DIR} FILES ${_headers})
	project_assign_source_group(${CMAKE_CURRENT_SOURCE_DIR} "" ${_sources_and_headers})
	project_assign_source_group(${CMAKE_CURRENT_SOURCE_DIR} "" ${_headers})

	set(${_out_sources_and_headers} ${_sources_and_headers} PARENT_SCOPE)
	set(${_out_headers} ${_headers} PARENT_SCOPE)
endfunction()

#######################################################################
# Convenience function of add_library
# project_add_library()
function(project_add_library )
  project_get_target_name(_target_name)
  project_add_prep(_sources_and_headers)

  # message(STATUS "Adding library ${_target_name}")
  # message(STATUS "  Sources: ${_sources_and_headers}")
...
#######################################################################
# Check if the target is buildable or not
# project_is_target_buildable()
function(project_is_target_buildable _result)
  ...
  set(_result_local ON)
	...
	set(${_out_build} ${_out_build_local} PARENT_SCOPE)
endfunction()

#######################################################################
# Continue CMake processing on any immediate subdirectories constaining CMakeLists.txt
...
function(project_append_target_property _target_name _property_name)
	...
	set_target_properties(${_target_name} PROPERTIES ${_property_name} "${_property}")
endfunction()

########################################################################
# Runs target_link_libraries on a target with the current directory name.
# project_target_link_libraries(<dependency name(s)> <additional optional params>)
function(project_target_link_libraries)
  ...
  foreach(_other_target IN ITEMS ${ARGN})
    ...
    target_link_libraries(${_target_name} ${_other_target})
		...
		# Skip public/private/interface keywords
		# WARNING: This is not a complete solution, and other keywords are permitted in target_link_libraries. May need to revisit this.
		if(${_other_target} STREQUAL "PUBLIC" OR ${_other_target} STREQUAL "INTERFACE" OR ${_other_target} STREQUAL "PRIVATE")
			continue()
		endif()
		project_append_target_property(${_target_name} PROJECT_DEPENDENCY_TARGETS ${_other_target})
		get_target_property(_dependency_targets ${_target_name} PROJECT_DEPENDENCY_TARGETS)
	endforeach()
endfunction()
...
function(project_add_executable_prep _is_unit_test)
  project_get_target_name(_target_name)
	...
	project_get_target_name(_target_name)
	set_property(GLOBAL APPEND PROPERTY PROJECT_EXECUTABLE_TARGETS ${_target_name})
...
########################################################################
# Creates an executable project for all source and header files in the current directory.
...
########################################################################
# Creates an example project for all source and header files in the current directory.
# project_add_example()
...
########################################################################
# Creates an unit test project for all source and header files in the current directory.
# project_add_unit_tests()
...
########################################################################
# Executes the target executable after it's built, and fails the build if the execution fails (eg. for unit tests)
# project_run_target_on_post_build()
function(project_run_target_on_post_build)
  
	if (BUILD_UNIT_TESTS)
		project_get_target_name(_target_name)
    add_custom_command(TARGET ${_target_name} POST_BUILD COMMAND ${_target_name}})
  endif()
endfunction()
